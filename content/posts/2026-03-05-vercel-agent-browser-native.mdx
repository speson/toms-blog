...
여기서 <strong>Node.js와 Playwright는 사실 오버헤드</strong>입니다. AI 에이전트는 그냥 "버튼 클릭해줘"라고 말하는데, 왜 중간에 두 개의 런타임을 거쳐야 할까요?

Vercel은 이 문제를 <strong>Rust로 해결</strong>했습니다.
...
agent-browser는 처음부터 <strong>AI가 쓰기 쉽게</strong> 설계되었습니다.
...
AI는 이제 <strong>고유 ref (@e1, @e2)</strong>로 요소를 참조할 수 있습니다.
...
AI가 요소를 구별하기 쉽게 <strong>번호가 표시된 스크린샷</strong> 생성:
...
<strong>전용 Rust 바이너리 제공</strong>, Node.js 폴백도 지원.
...
agent-browser --native는 <strong>"AI가 쓰기 위한 브라우저 자동화"</strong>의 새로운 표준을 제시합니다.
...
Playwright는 여전히 훌륭하지만, <strong>AI 에이전트에게는 agent-browser가 더 자연스러운 선택</strong>일 것 같습니다.
